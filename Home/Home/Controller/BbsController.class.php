...
class BbsController extends IndexController {
	...
	public function add_respond(){
		...
		$data['respond_post_id']=$post_id;
		$data['respond_ip']=getip();
		$r=M('Bbs_respond');
		$count=$r->add($data);
		if($count>0){
			$m=M('User');
	...
	public function add_do(){
		...
		if($count>0){
			...
			$this->success("发表成功，金币+{$this->status_arr[0]['s_money']}，积分+{$this->status_arr[0]['s_integral']},！",U('Index/index'),1);	
		}
		else{
			$this->error('发表失败!');
			exit();
