...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 8 );++$i;?><article class="excerpt">
<header><h2>
	    <!--<?php if($mod+1 <= 3): ?><img src="/Public/images/pin_<?php echo ($mod+1); ?>.gif"><?php endif; ?>-->
	<?php if($vo['post_status'] == 7): ?>【精华】<?php endif; ?>
		<a target="_blank" href="/index.php/Home/bbs/read/id/<?php echo ($vo["post_id"]); ?>.html" title="<?php echo ($vo["post_title"]); ?>"><?php echo ($vo["post_title"]); ?></a></h2>
	</header>
	<div class="focus">
		<a target="_blank" href="/index.php/Home/bbs/read/id/<?php echo ($vo["post_id"]); ?>.html">
			<?php
 $user=M('User'); $id=$vo['post_ower_id']; $user_img=$user->where("user_id='$id'")->getField('user_img'); if($user_img==''){ echo '<img class="image" src="/Public/Uploads/user_head/defalut.jpg" alt="'.$vo['post_title'].'" />'; }else{ echo '<img class="image" src="/Public/Uploads/user_head/'.$user_img.'" alt="'.$vo['post_title'].'" />'; } ?>
		</a>
	</div>
	<span class="note">
				<?php
 $content=$vo['post_content']; $content=preg_replace('/\<.*\>/uisU'," ",$content); echo $content= htmlspecialchars(sub_str($content,0,120,0)).'....'; ?>
		……<a href="/index.php/Home/bbs/read/id/<?php echo ($vo["post_id"]); ?>.html" rel="nofollow" class="more-link">继续阅读 &raquo;</a></span>
<p class="auth-span">
<span class="muted"><i class="fa fa-user"></i> 
	<a href="/index.php/Home/Home/zone/id/<?php echo ($vo["post_ower_id"]); ?>.html"><?php echo ($vo["post_ower_name"]); ?></a>
</span>
<span class="muted"><i class="fa fa-clock-o"></i>
	<?php echo ($vo["post_date"]); ?>
		...
		<span class="muted">
			<i class="fa fa-eye"></i>
				<span class="count"><?php echo ($vo["post_hot"]); ?></span>人看过
		</span></p>
